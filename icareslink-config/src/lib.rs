...
const CONFIG_FILENAME: &str = "icareslink.conf.toml";
...
pub fn config() -> &'static Config {
    ...
    let config_dir = CONFIG_DIR.get_or_init(|| {
        #[cfg(unix)]
        return PathBuf::from("/etc/icareslink");
        #[cfg(windows)]
        {
            return program_data_path.join("icareslink");
        }
    });

    let _ = LOG_DIR.get_or_init(|| {
        #[cfg(unix)]
        return PathBuf::from("/var/log/icareslink");
        #[cfg(windows)]
        return program_data_path.join("icareslink").join("log");
    });

    let _ = SOCKET_PATH.get_or_init(|| {
        #[cfg(unix)]
        return PathBuf::from("/var/run/icareslink.sock");
        #[cfg(windows)]
        return PathBuf::from("//./pipe/icareslink");
    });
    ...
    CONFIG.get_or_init(|| {
        Figment::from(Serialized::defaults(Config::default()))
            ...
            .merge(Env::prefixed("ICARESLINK_"))
            .extract()
            .unwrap()
    })
}
...
impl Default for Config {
    fn default() -> Self {
        Self {
            ...
            daemon_log_filename: "icareslink-daemon.log".into(),
            // IP of api.upvpn.app
...
impl Config {
    ...
    pub fn db_url(&self) -> String {
        format!("sqlite://{}/icareslink.db?mode=rwc", self.db_dir().display())
    }
    ...
    pub fn license_file_path(&self) -> PathBuf {
        ...
        #[cfg(target_os = "linux")]
        return PathBuf::from_str("/opt/icareslink/icareslink-oss-licenses.html").unwrap();
        #[cfg(target_os = "macos")]
        return PathBuf::from_str(
            "/Applications/icareslink.app/Contents/Resources/icareslink-oss-licenses.html",
        )
        .unwrap();
        ...
        .join("icareslink")
        .join("icareslink-oss-licenses.html");
    }

    pub fn icon_path(&self) -> &'static str {
        #[cfg(target_os = "linux")]
        return "/usr/share/icons/hicolor/32x32/apps/icareslink.png";
        #[cfg(target_os = "macos")]
