...
pub mod proto {
    tonic::include_proto!("icareslink.controller");
}
...
use icareslink_types::DateTimeUtc;
...
pub async fn new_grpc_client() -> Result<ControllerServiceClient, ControllerError> {
    let ipc_path = icareslink_config::config().socket_path();
...
pub async fn spawn_grpc_server<S, F>(//<S, P, F>
    service: S,
    // auth: P,
    shutdown: F,
    ...
    // P: Auth + 'static,
{
    ...
    let socket_path = icareslink_config::config().socket_path();
